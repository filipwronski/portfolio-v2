...
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push(function () {
  ...
  const _selectorExp = /(^[#\.]|[a-y][a-z])/gi
  const _commands = /[achlmqstvz]/ig
  ...
  const TweenLite = _gsScope._gsDefine.globals.TweenLite
  ...
  const _normalizedArcToBeziers = function (angleStart, angleExtent) {
    const segments = Math.ceil(Math.abs(angleExtent) / 90)
    let l = 0
    const a = []
    let angleIncrement; let controlLength; let angle; let dx; let dy; let i
    angleStart *= _DEG2RAD
  ...
  const _arcToBeziers = function (lastX, lastY, rx, ry, angle, largeArcFlag, sweepFlag, x, y) {
    if (lastX === x && lastY === y) {
      return
    }
    rx = Math.abs(rx)
    ry = Math.abs(ry)
    const angleRad = (angle % 360) * _DEG2RAD
    ...
    let p = ux
    sign = (uy < 0) ? -1 : 1
    let angleStart = (sign * Math.acos(p / n)) * _RAD2DEG

    n = Math.sqrt((ux * ux + uy * uy) * (vx * vx + vy * vy))
    p = ux * vx + uy * vy
    sign = (ux * vy - uy * vx < 0) ? -1 : 1
    let angleExtent = (sign * Math.acos(p / n)) * _RAD2DEG
    ...
    const bezierPoints = _normalizedArcToBeziers(angleStart, angleExtent)
    const a = cosAngle * rx
    const b = sinAngle * rx
    const c = sinAngle * -ry
    const d = cosAngle * ry
    const l = bezierPoints.length - 2
    let i; let px; let py
  ...
  const _pathDataToBezier = function (d) {
    const a = (d + '').replace(_scientific, function (m) { const n = +m; return (n < 0.0001 && n > -0.0001) ? 0 : n }).match(_svgPathExp) || [] // some authoring programs spit out very small numbers in scientific notation like "1e-5", so make sure we round that down to 0 first.
    const path = []
    let relativeX = 0
    let relativeY = 0
    const elements = a.length
    let l = 2
    let points = 0
    let i; let j; let x; let y; let command; let isRelative; let segment; let startX; let startY; let difX; let difY; let beziers; let prevCommand
  ...
  const _subdivideBezier = function (bezier, quantity) {
    let tally = 0
    const max = 0.999999
    let l = bezier.length
    const newPointsPerSegment = quantity / ((l - 2) / 6)
    let ax; let ay; let cp1x; let cp1y; let cp2x; let cp2y; let bx; let by
    let x1; let y1; let x2; let y2; let i; let t
    for (i = 2; i < l; i += 6) {
  ...
  const _bezierToPathData = function (beziers) {
    let data = ''
    const l = beziers.length
    const rnd = 100
    let sl; let s; let i; let segment
    for (s = 0; s < l; s++) {
      segment = beziers[s]
  ...
  const _reverseBezier = function (bezier) {
    const a = []
    let i = bezier.length - 1
    let l = 0
    while (--i > -1) {
      a[l++] = bezier[i]
      a[l++] = bezier[i + 1]
    ...
    bezier.reversed = !bezier.reversed
  }
  const _getAverageXY = function (bezier) {
  ...
  const _getSize = function (bezier) { // rough estimate of the bounding box (based solely on the anchors) of a single segment. sets "size", "centerX", and "centerY" properties on the bezier array itself, and returns the size (width * height)
    const l = bezier.length
    let xMax = bezier[0]
    let xMin = xMax
    let yMax = bezier[1]
  ...
  const _getTotalSize = function (bezier) { // rough estimate of the bounding box of the entire list of Bezier segments (based solely on the anchors). sets "size", "centerX", and "centerY" properties on the bezier array itself, and returns the size (width * height)
    let segment = bezier.length
    let xMax = bezier[0][0]
    let xMin = xMax
    let yMax = bezier[0][1]
    let yMin = yMax
    let l; let x; let y; let i; let b
    while (--segment > -1) {
      b = bezier[segment]
      l = b.length
      for (i = 6; i < l; i += 6) {
  ...
  const _sortByComplexity = function (a, b) {
    return b.length - a.length
  }
  const _sortBySize = function (a, b) {
    const sizeA = a.size || _getSize(a)
    const sizeB = b.size || _getSize(b)
    return (Math.abs(sizeB - sizeA) < (sizeA + sizeB) / 20) ? (b.centerX - a.centerX) || (b.centerY - a.centerY) : sizeB - sizeA // if the size is within 10% of each other, prioritize position from left to right, then top to bottom.
  }
  const _offsetBezier = function (bezier, shapeIndex) {
    const a = bezier.slice(0)
    const l = bezier.length
    const wrap = l - 2
    let i; let index
  ...
  const _getTotalMovement = function (sb, eb, shapeIndex, offsetX, offsetY) {
    const l = sb.length
    let d = 0
    const wrap = l - 2
    let index; let i; let x; let y
    shapeIndex *= 6
    for (i = 0; i < l; i += 6) {
      index = (i + shapeIndex) % wrap
  ...
  const _getClosestShapeIndex = function (sb, eb, checkReverse) { // finds the index in a closed cubic bezier array that's closest to the angle provided (angle measured from the center or average x/y).
    const l = sb.length
    const sCenter = _getAverageXY(sb) // when comparing distances, adjust the coordinates as if the shapes are centered with each other.
    const eCenter = _getAverageXY(eb)
    const offsetX = eCenter[0] - sCenter[0]
    const offsetY = eCenter[1] - sCenter[1]
    let min = _getTotalMovement(sb, eb, 0, offsetX, offsetY)
    let minIndex = 0
    let copy; let d; let i
    for (i = 6; i < l; i += 6) {
  ...
  const _getClosestAnchor = function (bezier, x, y) { // finds the x/y of the anchor that's closest to the provided x/y coordinate (returns an array, like [x, y]). The bezier should be the top-level type that contains an array for each segment.
    let j = bezier.length
    let closestDistance = 99999999999
    let closestX = 0
    let closestY = 0
    let b; let dx; let dy; let d; let i; let l
    while (--j > -1) {
      b = bezier[j]
      l = b.length
      for (i = 0; i < l; i += 6) {
  ...
  const _getClosestSegment = function (bezier, pool, startIndex, sortRatio, offsetX, offsetY) { // matches the bezier to the closest one in a pool (array) of beziers, assuming they are in order of size and we shouldn't drop more than 20% of the size, otherwise prioritizing location (total distance to the center). Extracts the segment out of the pool array and returns it.
    const l = pool.length
    let index = 0
    const minSize = Math.min(bezier.size || _getSize(bezier), pool[startIndex].size || _getSize(pool[startIndex])) * sortRatio // limit things based on a percentage of the size of either the bezier or the next element in the array, whichever is smaller.
    let min = 999999999999
    const cx = bezier.centerX + offsetX
    const cy = bezier.centerY + offsetY
    let size; let i; let dx; let dy; let d
  ...
  const _equalizeSegmentQuantity = function (start, end, shapeIndex, map) { // returns an array of shape indexes, 1 for each segment.
    let dif = end.length - start.length
    const longer = dif > 0 ? end : start
    const shorter = dif > 0 ? start : end
    let added = 0
    const sortMethod = (map === 'complexity') ? _sortByComplexity : _sortBySize
    const sortRatio = (map === 'position') ? 0 : (typeof (map) === 'number') ? map : 0.8
    let i = shorter.length
    const shapeIndices = (typeof (shapeIndex) === 'object' && shapeIndex.push) ? shapeIndex.slice(0) : [shapeIndex]
    let reverse = (shapeIndices[0] === 'reverse' || shapeIndices[0] < 0)
    const log = (shapeIndex === 'log')
    let eb; let sb; let b; let x; let y; let offsetX; let offsetY
    if (!shorter[0]) {
      return
    }
    if (longer.length > 1) {
  ...
  const _pathFilter = function (a, shapeIndex, map, precompile) {
    const start = _pathDataToBezier(a[0])
    const end = _pathDataToBezier(a[1])
  ...
  const _buildPathFilter = function (shapeIndex, map, precompile) {
    return (map || precompile || shapeIndex || shapeIndex === 0) ? function (a) {
      _pathFilter(a, shapeIndex, map, precompile)
    } : _pathFilter
  }
  const _offsetPoints = function (text, offset) {
    if (!offset) {
      return text
    }
    const a = text.match(_numbersExp) || []
    const l = a.length
    let s = ''
    let inc; let i; let j
    if (offset === 'reverse') {
  ...
  const _equalizePointQuantity = function (a, quantity) {
    let tally = 0
    let x = parseFloat(a[0])
    let y = parseFloat(a[1])
    let s = x + ',' + y + ' '
    const max = 0.999999
    let newPointsPerSegment; let i; let l; let j; let factor; let nextX; let nextY
    l = a.length
  ...
  const _pointsFilter = function (a) {
    const startNums = a[0].match(_numbersExp) || []
    const endNums = a[1].match(_numbersExp) || []
    const dif = endNums.length - startNums.length
    if (dif > 0) {
  ...
  const _buildPointsFilter = function (shapeIndex) {
    return !isNaN(shapeIndex) ? function (a) {
      _pointsFilter(a)
      a[1] = _offsetPoints(a[1], parseInt(shapeIndex, 10))
    } : _pointsFilter
  }
  const _createPath = function (e, ignore) {
    const path = _gsScope.document.createElementNS('http://www.w3.org/2000/svg', 'path')
    const attr = Array.prototype.slice.call(e.attributes)
    let i = attr.length
    let name
    ignore = ',' + ignore + ','
    while (--i > -1) {
  ...
  const _convertToPath = function (e, swap) {
    const type = e.tagName.toLowerCase()
    const circ = 0.552284749831
    let data; let x; let y; let r; let ry; let path; let rcirc; let rycirc; let points; let w; let h; let x2; let x3; let x4; let x5; let x6; let y2; let y3; let y4; let y5; let y6
  ...
  const _parseShape = function (shape, forcePath, target) {
    const isString = typeof (shape) === 'string'
    let e; let type
  ...
  const _morphMessage = 'Use MorphSVGPlugin.convertToPath(elementOrSelectorText) to convert to a path before morphing.'

  var MorphSVGPlugin = _gsScope._gsDefine.plugin({
    propName: 'morphSVG',
    API: 2,
    global: true,
    version: '0.8.11',
    ...
    init (target, value, tween, index) {
      let type, p, pt, shape, isPoly
  ...
  MorphSVGPlugin.convertToPath = function (targets, swap) {
    ...
    const a = (!targets || targets.length === 0) ? [] : (targets.length && targets[0] && targets[0].nodeType) ? Array.prototype.slice.call(targets, 0) : [targets]
    let i = a.length
    while (--i > -1) {
  ...
  MorphSVGPlugin.pathDataToBezier = function (data, vars) { // converts SVG path data into an array of {x, y} objects that can be plugged directly into a bezier tween. You can optionally pass in a 2D matrix like [a, b, c, d, tx, ty] containing numbers that should transform each point.
    const bezier = _pathDataToBezier(_parseShape(data, true))[0] || []
    let prefix = 0
    let a; let i; let l; let matrix; let offsetX; let offsetY; let bbox; let e
    vars = vars || {}
